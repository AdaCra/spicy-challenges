...
document.addEventListener("scroll", () => {
  let styleInputValue = calculateScrollPercentage(window.scrollY);
  progressBar.style.width = styleInputValue + "%";
});

function calculateScrollPercentage(scrollPosition) {
  const result =
    (scrollPosition / (document.body.clientHeight - window.innerHeight)) * 100;
  return result;
}
